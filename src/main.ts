...
import {SortGuiView, SORT_GUI_VIEW_TYPE} from "./sort-gui/SortGuiView";
	...
	implements CustomSortPluginAPI
{
	...
	async onload() {
		...
		// Register the sort GUI view
		this.registerView(
			SORT_GUI_VIEW_TYPE,
			(leaf) => new SortGuiView(leaf, this)
		);
		...
		if (Platform.isDesktop) {
			this.ribbonIconEl = this.addRibbonIcon('list-ordered', 'Open Drag & Drop Sort', () => {
				...
				if (existingLeaf) {
					...
					if (leaf) {
						leaf.open(new SortGuiView(leaf, this));
					}
				}
			});
		}
	...
	registerCommands() {
		...
		this.addCommand({
			id: 'open-drag-drop-sort',
			name: 'Open drag & drop sort GUI',
			callback: () => {
				const existingLeaf = this.app.workspace.getLeavesOfType(SORT_GUI_VIEW_TYPE).first();
				if (existingLeaf) {
					this.app.workspace.revealLeaf(existingLeaf);
